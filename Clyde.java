...
public class Clyde {
    ...
    public Clyde(int xPos, int yPos, int clyDirection) {
        this.direction = clyDirection;
        this.xPos = xPos;
        this.yPos = yPos;
        this.fright = false;
    }
    ...
    public void scatter(ArrayList<Wall> listOfWalls, ArrayList<Point> intersections) {
        //for this, its the same logic for the red ghost targetting scheme, however the target tile is the corner of the map
    ...
    public void chase(int targetX, int targetY, ArrayList<Wall> listOfWalls, ArrayList<Point> intersections) {
        ...
        boolean isUpCollision = checkWallCollision(listOfWalls, 0);
        boolean isDownCollision = checkWallCollision(listOfWalls, 1);
        double distance = Math.sqrt(Math.pow(xPos - targetX, 2) + Math.pow((yPos - targetY), 2));
        if (distance <= MAXTILE) {

            scatter(listOfWalls, intersections);
        } else {
            if (isIntersection) {
                if (!isRightCollision && this.direction != 3) {
