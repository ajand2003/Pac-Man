...
public class Inky {
    ...
    private final int SCATTERX = 25;
    private final int SCATTERY = 785;
    private final int HOMEX = 375;
    private final int HOMEY = 445;
    private boolean fright;

    public Inky(int x, int y, int direction) {
        this.xPos = x;
        this.yPos = y;
    ...
    public void chase(int targetX, int targetY, int pacDirection, ArrayList<Wall> listOfWalls, ArrayList<Point> intersections, Blinky bli) {
        double upDistance = 500000000.0;
        double downDistance = 5000000.0;
        double leftDistance = 50000000.0;
        double rightDistance = 5000000.0;
        ...
        targetX = targetX - bli.getxPos() + targetX;
        targetY = targetY - bli.getyPos() + targetY;

        double distance = Math.pow((bli.getxPos() - targetX), 2) + Math.pow((bli.getyPos() - targetY), 2);
        if (isIntersection) {
            if (!isRightCollision && this.direction != 3) {
